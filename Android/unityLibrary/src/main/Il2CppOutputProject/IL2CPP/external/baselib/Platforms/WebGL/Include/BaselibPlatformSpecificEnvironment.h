...
// sem_t defined in semaphore.h as struct { volatile int __val[4*sizeof(long)/sizeof(int)];};
enum { Baselib_SystemSemaphore_PlatformSize = sizeof(int)*4*sizeof(long)/sizeof(int) };
#else
// namespace detail { struct Semaphore { uint32_t counter; }; }
// size should match size of the struct above from no-thread implementation
