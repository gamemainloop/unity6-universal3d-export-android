...
#ifdef FEATURE_RUNTIME_SHUTDOWN
static pthread_t s_handlerThread;
static volatile bool s_handlerThreadCreated;
#endif
...
#ifdef FEATURE_RUNTIME_SHUTDOWN
        if (!s_handlerThreadCreated) // If s_handlerThreadCreated is false when we get here then we are exiting
            return NULL;
#endif
...
#ifdef FEATURE_RUNTIME_SHUTDOWN
void UninitializeSignalHandlingCore(void)
{
    pthread_mutex_lock(&lock);

    if (s_handlerThreadCreated)
    {
        // Set this before writing to the signalPipe
        // The s_handlerThread will wake up after reading from the pipe and this need to be
        // cleared for it to exit
        __atomic_clear(&s_handlerThreadCreated, __ATOMIC_RELEASE);

        void* value = NULL;
        write(g_signalPipe[1], &value, sizeof(uint8_t));

        // Wait for the handler thread to exist before freeing anything else
        pthread_join(s_handlerThread, &value);

        // Do not close the pipe until s_handlerThread has exited
        // The thread reads from read pipe.  It is not safe to close a pipe while a call to read() is in process
...
void SystemNative_UninitializeTerminalAndSignalHandling(void)
{
    if (pthread_mutex_lock(&lock) == 0)
    {
        UninitializeSignalHandlingCore();
        pthread_mutex_unlock(&lock);
    }
}
#endif
