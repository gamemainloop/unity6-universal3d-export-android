...
#if !HAVE_O_CLOEXEC && HAVE_FD_CLOEXEC
...
{
    // Using HAVE_FD_CLOEXEC as a proxy for F_SETFD (Currently FD_CLOEXEC is the only flag F_SETFD supports)
    // Currently the runtime only uses F_SETFD to clear FD_CLOEXEC to allow inheretance
#if HAVE_FD_CLOEXEC
    int result;
    while ((result = fcntl(ToFileDescriptor(fd), F_SETFD, ConvertOpenFlags(flags))) < 0 && errno == EINTR);
    return result;
#else
    // Currently the only platforms that don't support FD_CLOEXEC don't support process inheretance and the classlibs
    // do an F_SETFD to zero to clear FD_CLOEXEC. So this a no-op since the handle can't be inhereted
    // Return 0 for this case to prevent an needless(?) exception
    if (flags == 0)
        return 0;
    errno = ENOTSUP;
    return -1;
...
{
    // Using HAVE_FD_CLOEXEC as a proxy for F_GETFD (Currently FD_CLOEXEC is the only flag F_GETFD supports)
#if HAVE_FD_CLOEXEC
