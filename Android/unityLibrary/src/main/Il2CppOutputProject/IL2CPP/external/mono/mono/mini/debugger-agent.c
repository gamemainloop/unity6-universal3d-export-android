...
#define DECLARE_IL2CPP_GENERIC_CONTEXT Il2CppGenericContext il2cpp_generic_context;
#define IL2CPP_GENERIC_CONTEXT_PARAM , &il2cpp_generic_context
#else
#define DECLARE_IL2CPP_GENERIC_CONTEXT
#define IL2CPP_GENERIC_CONTEXT_PARAM
#endif
...
decode_value (MonoType *t, MonoDomain *domain, gpointer void_addr, gpointer void_buf, guint8 **endbuf, guint8 *limit, gboolean check_field_datatype)
{
	...
	if (mono_type_get_type(t) == MONO_TYPE_GENERICINST && mono_class_is_nullable (mono_class_from_mono_type_internal (t))) {
		DECLARE_IL2CPP_GENERIC_CONTEXT
		MonoType *targ = VM_GENERIC_INST_TYPE_ARGV(mono_generic_class_get_context (m_type_get_generic_class (t) IL2CPP_GENERIC_CONTEXT_PARAM)->class_inst, 0);
		guint8 *nullable_buf;
...
{
#ifdef RUNTIME_IL2CPP
	MonoObject* o = il2cpp_debugger_assembly_get_object(domain, ass, error);
	if (o == NULL) {
		return ERR_INVALID_OBJECT;
	}
	buffer_add_objid(buf, o);
	...
	case CMD_TYPE_GET_INFO: {
		...
		if (CHECK_PROTOCOL_VERSION (2, 15)) {
			...
			if (mono_class_is_ginst (klass)) {
				DECLARE_IL2CPP_GENERIC_CONTEXT
				MonoGenericInst *inst = mono_generic_class_get_context (mono_class_get_generic_class (klass) IL2CPP_GENERIC_CONTEXT_PARAM)->class_inst;
...
method_commands_internal (int command, MonoMethod *method, MonoDomain *domain, guint8 *p, guint8 *end, Buffer *buf)
{
			...
			if (method->is_generic || method->is_inflated) {
				...
				if (method->is_generic) {
					...
					if (VM_INFLATED_METHOD_GET_CLASS_INST(imethod)) {
						...
						DECLARE_IL2CPP_GENERIC_CONTEXT
						if (mono_class_get_context (klass IL2CPP_GENERIC_CONTEXT_PARAM )) {
							ERROR_DECL (error);
							result = mono_class_inflate_generic_method_full_checked (result, klass, mono_class_get_context (klass IL2CPP_GENERIC_CONTEXT_PARAM), error);
							if (!is_ok (error)) {
								add_error_string (buf, mono_error_get_message (error));
								mono_error_cleanup (error);
								return ERR_INVALID_ARGUMENT;
	...
	case CMD_METHOD_MAKE_GENERIC_METHOD: {
		...
		g_free (type_argv);
		DECLARE_IL2CPP_GENERIC_CONTEXT
		tmp_context.class_inst = mono_class_is_ginst (method->klass) ? mono_generic_class_get_context (mono_class_get_generic_class (method->klass) IL2CPP_GENERIC_CONTEXT_PARAM)->class_inst : NULL;
